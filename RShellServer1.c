...
// Of course, if the authentication fails then return false
	...
	//file to open password text file 
	FILE *passFile;

	//open file in read mode
	passFile = fopen(pwdfname, "r");
	...
	//variable to hold username from file
	char* username;

	//variable to hold password from file 
	char* password;

	//buffer to read from file
	char *buffer = NULL;

	//buffer length for reading
	size_t bufSize = 0;

	//variable for getline
	size_t characters;

	//get the single line from the file
	characters = getline(&buffer, &bufSize, passFile);

	//buffer to find values from the line
	char* pol;

	//read line up to the ';' char
	pol = strtok(buffer, ";");

	//copy value into username
	memcpy(&username, &pol, sizeof(username));
	...
	//copy value over
	memcpy(&password, &pol, sizeof(password));
	...
	//compare if client/server IDs match
	if(strcmp(cluser, username) != 0)
	{
	...
	//if both values match then authentication is successful
