...
/*
        // ADD YOUR CODE HERE
        // Use the provided SHA1 code SHA1 hashing of the password
...
*/
	...
	//calculate tmphash using sha1 from provided files
	SHA1(tmphash, argv[4], strlen(argv[4]));

	//formate hash for comparison
	for(int offset = 0; offset < 20; offset++)
	{
		sprintf((password + (2 * offset)), "%02x", tmphash[offset]&0xff);
	}
        // Setting a dummy value for password.  Replace this with the value you calculated!!
        //char str_fake[20] = "FAKE FAKE";
        //strcpy(password, str_fake);
